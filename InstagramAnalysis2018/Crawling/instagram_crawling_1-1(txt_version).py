# instagram_crawling_1-1(txt_version).py
# 인스타그램 셀레니움으로 크롤링
...
if(len(arr_href) < 10):      # 숫자는 아무상관없음. 그냥 파일에 게시글주소 몇개 들어있는지 보는 것.
    ...
    # 게시물 주소 저장
    f = open(".\\instagrampost.txt", "w", encoding='utf-8')
    s = ''
...
for h in arr_href:
    ...
    text = ''
    i=0
    for i in range(0,3):                                                                    # 댓글을 위에서부터 두개까지만 받아올 것.
        try:
