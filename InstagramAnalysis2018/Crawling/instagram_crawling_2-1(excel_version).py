...
# 인스타그램 셀레니움으로 크롤링
# 1. 각자가 쓴 인스타그램 게시글로 들어갈 수 있는 url 받기       : instagram_crawling_2-1(excel_version).py
# 2. 각 url로 들어가 그곳에서 아이디, 태그 or 글 뽑기            : instagram_crawling_2-1(excel_version).py
# 3. 뽑은 태그 or 글을 txt파일로 저장하기.                       : instagram_crawling_2-1(excel_version).py

# 4. 뽑은 아이디로 각자의 프로필화면으로 들어가 프로필 다운받기. : instagram_crawling_2-2(excel_version).py
...
for h in arr_href:
   ...
   href[i].find_element_by_css_selector('span').text            # 댓글이 그냥 텍스트 내용이라면 여기서 검출될 것.(각 댓글중 0번째와 1번째만 봄.)
